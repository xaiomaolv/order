...
/**
 *
 *  {"cardType":"DC","bank":"ICBC","key":"6212262502022006756","messages":[],"validated":true,"stat":"ok"}
 * @param {Object} cardNo
 */
export async function checkCard (cardNo) {
  const url = 'https://ccdcapi.alipay.com/validateAndCacheCardInfo.json?_input_charset=utf-8&cardNo=' + cardNo + '&cardBinCheck=true';
  ...
  return await axios.get(url).then(response => {
    ...
    if (!response.data.validated) {
      ...
      let bankName = getBankInfo(response.data.bank)
      let cardTypeName = getCardTypeName(response.data.cardType)
     json = response.data;
      json['bankName'] = bankName
      json['cardTypeName'] = cardTypeName
      return json;
    }
...
function getBankInfo (bank) {
  if (bankcardList[bank]) {
    return bankcardList[bank]
  }
  return undefined
}
