              ...
              >
                ...
                <template #price>
                  ...
                  <span v-else class="mg-lt-10 ft-se-12"
                    >￥{{
                      item.productPercentPrice + item.addPrice || ""
                    }}</span
                  >
                </template>
              </van-card>
...
export default {
  ...
  methods: {
    ...
    loadHref() {
      //   if (isWechat) {
      // let appid = "wx28013f88973fe172"; //微信APPid  appid 测试号：wxe744ba8e6a137e71；   恩捷：wx28013f88973fe172
      let appid = process.env.VUE_APP_APP_ID;
      let code = this.getUrlCode().code; //是否存在code
      let state = this.getUrlCode().state;
      console.log("code=" + code + "   ____state=" + state);
      let local = window.location.href;
      //   let local = "https://www.baidu.com/";
      if (code == null || code === "" || state == null || state == "") {
        console.log("code == null || code === ''");
        //不存在就打开上面的地址进行授权
        ...
        this.param.wxCode = code;
        this.orderId = state;
        console.log("code和orderId的值：" + code + "------------" + state);
        this.getOrder();
      }
    },
    getUrlCode() {
      ...
      //   var url ="https://www.baidu.com/?orderId=1482334670241763417&code=0114kJ0w3BPOEY2KZP2w3YHM4734kJ00&state=1482334670241763471";
      console.log("url--------" + url);
    ...
    getOrder() {
      ...
      authOrderInfo(this.param).then((res) => {
        console.log("订单信息：" + res);
        if (res.data.flag == 1) {
          this.$router.push({ path: "/login" });
          //   Toast.fail("没有权限查看该订单信息！");
        } else {
      ...
      getOrderInfo(orderId).then((res) => {
        if (res.code == 200) {
          ...
          if (res.data.customerName) {
            this.customerName =
              res.data.customerName + "(" + res.data.customerPhone + ")";
          }
