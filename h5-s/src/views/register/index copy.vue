<template>
  <div class="container">
        ...
        <van-tabs v-model="active">
          <van-tab>
            <van-form class="login-form cellGroup" ref="formZero">
              ...
              <van-row class="mg-tp-20 ft-se-14 mg-lt-20">
                <van-col>
                  <van-checkbox v-model="checked" checked-color="#C83C3C"
                    >我已阅读并同意</van-checkbox
                  >
                </van-col>
                <van-col>
                  <i
                    style="color: #c83c3c; margin-top: 40px"
          ...
          <van-tab>
            <van-form class="login-form" ref="formOne">
              ...
              >
                <template #button>
                  <van-icon name="location-o" color="#ff976a" @click="showMap = true"/>
                </template>
              </van-field> -->
              <van-field size="large" name="address" label="  ">
                <template #input>
                  <van-button
                    size="small"
                    class="button"
          ...
          <van-tab>
            <van-form class="login-form" ref="formTwo">
              ...
              <van-field
                v-if="infoForm.specialQualification"
                v-model="infoForm.thirdImg"
                size="large"
                name="thirdImg"
                label="证件图片"
                :rules="typeImgThree"
    ...
    <van-popup v-model="showPopup" :style="{ height: '70%', width: '90%' }">
      <van-row class="pd-tb-10 tx-ct">《彩云点餐商家版用户协议》
        <Aagreement></Aagreement>
      </van-row>
    </van-popup>
...
import Aagreement from "./agreement.vue";
  ...
  methods: {
    ...
    sendSms() {
      ...
      let params = {
        ...
        code: that.infoForm.captCode,
        uuid: that.infoForm.code,
      };
      registerSend(params).then((res) => {
        ...
        if (res.data.code == 2) {
          ...
          this.infoForm.code = res.data.code;
    ...
    afterReadD(file) {
      ...
      //     .then((res) => {
      //       if (res.code == 200) {
      //         this.infoForm.secondtypeImg = res.data.fileName;
      //         file.status = "success";
      //         file.message = "上传成功";
      //       }
      //     })
    ...
    afterReadE(file) {
      ...
      //     .then((res) => {
      //       if (res.code == 200) {
      //         this.infoForm.thirdImg = res.data.fileName;
      //         file.status = "success";
      //         file.message = "上传成功";
      //         console.log(this.infoForm, "thirdImg");
      //       }
      //     })
      //     .catch((err) => {
      //       file.status = "failed";
    ...
    afterReadF(file) {
      ...
      //     .then((res) => {
      //       if (res.code == 200) {
      //         this.infoForm.identityFrontPhoto = res.data.fileName;
      //         file.status = "success";
      //         file.message = "上传成功";
      //       }
      //     })
    ...
    afterReadG(file) {
      ...
      //         this.infoForm.identityBackPhoto = res.data.fileName;
      //         file.status = "success";
      //         file.message = "上传成功";
      //       }
      //     })
    ...
    onSubmit() {
      ...
      this.$refs.formThree
        .validate()
        .then(() => {
          addBusiness(this.infoForm).then((res) => {
            if (res.code == 200) {
              this.$message({
                // message: "注册成功，请登录店铺进行设置",
                dangerouslyUseHTMLString: true,
                message: "<h3>注册成功，请耐心等待审核。</h3>",
                type: "success",
              });
    ...
    onSubmitTree() {
      ...
      let categoryList = this.category
        ? this.category.split(",")
        : this.infoForm.businessCategory.split(",");
