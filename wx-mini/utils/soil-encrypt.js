...
function sm4Encrypt(content) {
    let key = miniConfig.APP_SM4_KEY
    // 不存在rsaKey直接返回
    if (!key) {
      return content
    }
  ...
  function sm2Sign(word) {
    let prikey = miniConfig.APP_PRI_KEY
    if (!prikey) {
      return word
    }
  ...
  function sm2VerifySign(word, signStr) {
    let pubKey = miniConfig.APP_SERVER_PUB_KEY
    // 不存在rsaKey直接返回
    if (!pubKey) {
      return word
    }
  ...
  export const decryptData = function (res) {
    ...
    if (miniConfig.APP_SIGN == 'true' && datade.code==200 && (!data.sign || !sm2VerifySign(JSON.stringify(datade), data.sign))) {
  ...
  export const sign = function(config,paramDataStr) {
  ...
  //解密验签方法
  export const  decryptVerifySign = function(res){
    let data = Object.assign({}, res)
  
    let datade = (miniConfig.APP_ENCRYPTION == 'true' && data.enData) ? JSON.parse(sm4Decrypt(data.enData)) : data
    //配置签名情况下，若返回失败信息可以不验签
    if (miniConfig.APP_SIGN == 'true' && datade.code==200 && (!data.sign || !sm2VerifySign(JSON.stringify(datade), data.sign))) {
      console.error('decryptData', res, "验签失败！")
