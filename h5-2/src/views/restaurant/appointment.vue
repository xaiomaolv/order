<template>
	<div class="container ps-re">
				...
				<div class="card-view bg-w mg-lr-10">
					...
					<div class="pd-tb-10">
						<van-field v-model="mealTime" required name="mealTime" input-align="right" label="用餐时间"
							placeholder="请选择用餐时间" @click="showTime = true" readonly />
		...
		<van-popup v-model:show="showTime" position="bottom" @close="onClose">
			<van-picker :columns="columns" @confirm="onConfirm" @cancel="showPicker = false"   show-toolbar/>
		</van-popup>
...
<script>
	...
	export default {
		...
		methods:{
			...
			onConfirm(val){
				this.mealTime = val[0] + val[1] + val[2];
				this.timeStr = val[0] + val[1].replace('时', '') + ':' + val[2].replace('分', '')
				this.showTime = false;
			},
			...
			appointment(){
				...
				let params = {
					businessId: this.storeInfo.businessId, //商家id
					orderType: "0", //** 是否提前点餐  0 否 1. 是*/
					userId: getLocal('userId'), //用户id
					reserveName: this.userName + this.userSex,
					userPhone: this.userPhone, //用户电话
					isDeposit: "0", //** 是否已交订金  0 未交 1. 已交*/
					deposit: 0, //预约定金金额
					mealTime: this.timeStr, //就餐时间
					mealPerson: this.mealPerson, //就餐人数
				};
				
				addResverse(params).then(res => {
					if (res.code == 200) {
					 Toast.success('预约成功')
			
						this.mealTime = null,
						this.timeStr = ''
						this.mealPerson = null,
